...
data.file<-"raw_data/Trauma_clean_update_20240508_95CI.csv"
data.file
...
colnames(data.melt.biomarker)[c(118,119)]=c("biomarker","expression")
...
ggplot(data.melt.biomarker,
       aes(y=.data[[colnames(data.melt.biomarker)[y.No]]],
                  x=.data[[colnames(data.melt.biomarker)[x.No]]] 
...
stat.test2 <- stat.test2 %>%
  #Auto-compute p-value label positions using the function, in rstatix package
  add_xy_position(x = "biomarker")
...
pdf ("biomarker_panel_severity_vertical_without sig1.pdf",
     width = 6,height = 8)
...
ggplot(data.melt.biomarker[data.melt.biomarker$Group %in% c("Trauma","Control"),],
       aes(
         x=.data[[colnames(data.melt.biomarker)[x.No]]], 
         y=.data[[colnames(data.melt.biomarker)[y.No]]]
       )
)+
  geom_violin(aes_string(fill=colnames(data.melt.biomarker)[fill.No]),
